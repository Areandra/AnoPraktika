...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $myPracticums = $user->practicums()->withPivot('role', 'status')->get();
        ...
        if ($request->has('practicum_id')) {
            $activePracticum = $user->practicums()
                ...
                ->withPivot('role', 'status') // Pastikan status terbaca di sini
                ->withCount(['users as total_students' => function ($q) {
                    $q->where('role', 'student')->where('status', 'joined');
                }])
            ...
            if ($request->has('assignment_id')) {
                ...
                if ($request->get('show_users') == 'true') {
                    $viewMode = 'list_users';
                    $centerData['users'] = $activePracticum->users()->withPivot('role', 'status')->get();
