...
<body class="bg-[#060b18] text-slate-200 h-[100dvh] overflow-hidden flex flex-col lg:flex-row" style="font-size: 13px;"
    ...
    <div class="fixed top-4 right-4 z-[100] flex flex-col gap-2 pointer-events-none" style="max-width: 360px;">
        @if (session('success'))
            <div
                class="toast-slide pointer-events-auto flex items-center gap-3 rounded-xl border border-emerald-500/25 bg-[#0a1f14]/90 backdrop-blur px-4 py-3 text-emerald-400 shadow-2xl shadow-black/40">
        ...
        @if ($errors->any())
            <div
                class="toast-slide pointer-events-auto flex flex-col gap-1.5 rounded-xl border border-red-500/25 bg-[#1a0a0a]/90 backdrop-blur px-4 py-3 text-red-400 shadow-2xl shadow-black/40">
    ...
    <div
        class="lg:hidden flex-shrink-0 flex items-center justify-between px-4 py-3 bg-[#0d1424] border-b border-slate-800/60 z-30">
        <button @click="mobileLeftOpen = true"
            class="p-2 -ml-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800/60 transition focus:outline-none">
            <i data-lucide="menu" class="w-5 h-5"></i>
        </button>

        <div class="flex items-center gap-2">
            <div
                class="w-6 h-6 rounded-md bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center shadow-lg shadow-indigo-500/25">
                <i data-lucide="layers" class="w-3 h-3 text-white"></i>
            </div>
            <span class="font-bold text-sm text-white tracking-tight">AnoPraktika</span>
        </div>

        <button @click="mobileRightOpen = true"
            class="p-2 -mr-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800/60 transition focus:outline-none">
            <i data-lucide="info" class="w-5 h-5"></i>
        </button>
    </div>

    {{-- LEFT SIDEBAR BACKDROP (Mobile) --}}
    <div x-show="mobileLeftOpen" x-transition.opacity @click="mobileLeftOpen = false"
    ...
    <div :class="mobileLeftOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
        ...
        <div class="lg:hidden absolute top-3 right-3">
            <button @click="mobileLeftOpen = false"
                class="p-1.5 rounded-lg text-slate-400 hover:text-white bg-slate-800/50 transition">
    ...
    <div x-show="mobileRightOpen" x-transition.opacity @click="mobileRightOpen = false"
        class="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 lg:hidden" x-cloak></div>
    ...
    <div :class="mobileRightOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'"
        ...
        <div class="lg:hidden absolute top-3 right-3 z-10">
            <button @click="mobileRightOpen = false"
                class="p-1.5 rounded-lg text-slate-400 hover:text-white bg-slate-800/50 transition border border-slate-700">
