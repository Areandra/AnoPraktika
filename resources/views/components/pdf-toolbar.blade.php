...
<div id="pdf-toolbar"
    ...
    <div class="flex items-center gap-2">
        ...
        <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/50 border border-slate-700/40">
            ...
            <span class="text-xs font-mono text-slate-300">
                <span id="pdf-current-page" contenteditable="true"
                    class="text-white font-bold outline-none focus:ring-0 min-w-[12px] inline-block text-center border-b border-transparent focus:border-slate-500 transition-colors"
                    onblur="jumpToPage(this.textContent)"
                    onkeydown="if(event.key==='Enter'){event.preventDefault();this.blur();}">0</span>
        ...
        <button onclick="toggleContinuousMode()" id="btn-continuous-mode"
            class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/50 border border-slate-700/40 text-xs font-mono text-slate-400 hover:text-white transition"
    ...
    <div class="flex items-center gap-1.5">
        <span class="hidden md:flex text-xs text-slate-600 font-mono mr-1">Zoom</span>
